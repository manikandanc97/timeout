  ...
  return (
    <section className='overflow-hidden rounded-2xl border border-border bg-card/90 p-4 shadow-sm backdrop-blur-sm md:p-5'>
      <div className='flex justify-between items-center gap-3'>
        <h3 className='text-lg font-semibold text-card-foreground'>Choose leave type</h3>
        <div className='flex items-center gap-2'>
          <div className='hidden sm:flex items-center gap-2'>
            <button
              ...
              className={`inline-flex h-8 w-8 items-center justify-center rounded-full border transition-colors ${
                canGoPrev ? 'border-border bg-card text-muted-foreground hover:bg-muted' : 'cursor-not-allowed border-border bg-muted text-muted-foreground/60'
              }`}
            >
              <ChevronLeft size={16} />
            </button>
            <button
              type='button'
              onClick={() => setLeaveTypeStart((prev) => Math.min(prev + 1, maxLeaveTypeStart))}
              disabled={!canGoNext}
              className={`inline-flex h-8 w-8 items-center justify-center rounded-full border transition-colors ${
                canGoNext ? 'border-border bg-card text-muted-foreground hover:bg-muted' : 'cursor-not-allowed border-border bg-muted text-muted-foreground/60'
              }`}
            >
          ...
          <div className='flex items-center gap-2 rounded-full bg-muted px-3 py-1 text-[11px] font-medium text-muted-foreground'>
            ...
            <span>{LEAVE_BALANCE_LABELS[type as LeaveType] ?? 'Balance aware'}</span>
          </div>
        </div>
      </div>
                ...
                return (
                  <Button
                    ...
                    className={`group relative flex w-[calc((100%-1.5rem)/3)] shrink-0 flex-col gap-3 rounded-xl border px-4 py-4 text-left !h-auto !text-card-foreground transition-all duration-150 ${
                      isSelected ? `${cfg.bg} ${cfg.border} border-2` : 'border-border !bg-muted/70'
                    }`}
                  >
                    {isSelected ? (
                      <div className={`absolute right-3 top-3 z-10 inline-flex h-5 w-5 translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full border border-card ${cfg.color} ${cfg.bg}`}>
                        <Check size={12} strokeWidth={3} />
                      </div>
                    ) : null}
                    <div className='flex items-start gap-3'>
                      <div className={`flex h-10 w-10 items-center justify-center rounded-lg border bg-card shadow-sm ${isSelected ? cfg.border : 'border-border'}`}>
                        <Icon size={18} className={isSelected ? cfg.color : 'text-muted-foreground'} />
                      </div>
                      <div className='space-y-1'>
                        <p className={`text-sm font-semibold leading-tight ${isSelected ? cfg.color : 'text-card-foreground'}`}>{cfg.label}</p>
                        <p className='text-xs text-muted-foreground'>{cfg.desc}</p>
                      </div>
                      {balanceLabel !== null ? (
                        <div className={`ml-auto items-center justify-center px-2 py-1 text-xs font-semibold ${isSelected ? `${cfg.border} ${cfg.color}` : 'border-border text-muted-foreground'}`}>
                          {balanceLabel}d
                        </div>
                      ) : null}
                    </div>

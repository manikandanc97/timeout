      ...
      <div className='relative z-10 flex flex-col gap-4 p-4 sm:p-5'>
        <div className='flex flex-wrap items-start justify-between gap-3'>
          <div className='flex items-start gap-3'>
            ...
            <div className='space-y-2'>
              <div className='h-2.5 w-20 animate-pulse rounded bg-muted/90' />
              <div className='h-8 w-40 animate-pulse rounded bg-muted/90' />
        ...
        <section className='grid w-full shrink-0 grid-cols-2 gap-3 sm:grid-cols-5 sm:gap-3.5'>
          {Array.from({ length: 5 }).map((_, i) => (
            ...
            >
              <div className='h-3 w-24 animate-pulse rounded bg-muted/90' />
              <div className='mt-2 h-8 w-16 animate-pulse rounded bg-muted/90' />
            </article>
          ))}
        </section>
        ...
        {Array.from({ length: 3 }).map((_, sectionIdx) => (
          ...
          >
            <div className='space-y-2'>
              <div className='h-5 w-52 animate-pulse rounded bg-muted/90' />
              ...
              <div className='sticky top-0 z-10 border-b border-border bg-muted/95 px-4 py-3 backdrop-blur-sm'>
                <div className='flex min-w-[980px] gap-3'>
                  {Array.from({ length: 8 }).map((__, i) => (
                    <div key={i} className='h-3 w-20 animate-pulse rounded bg-muted/80' />
                  ))}
                </div>
              </div>
                ...
                {Array.from({ length: 5 }).map((__, rowIdx) => (
                  <div key={rowIdx} className='flex min-w-[980px] items-center gap-3 px-4 py-3.5'>
                    {Array.from({ length: 8 }).map((___, colIdx) => (
                      <div
                        key={`${rowIdx}-${colIdx}`}
                        className='h-3.5 w-24 shrink-0 animate-pulse rounded bg-muted'
                      />
                    ))}
                  </div>
                ))}
